subroutine J_Pht_Env(snes,pht_vec,pht_jacob,pht_precond,dummy,ierr)
  use commondata
  use fields
  use laplacians
  use gradients
...
#include <finclude/petscsnes.h>
  ...
  do x = 1,psx
     do y = 1,psy
        do z = 1,psz+2
           D_pht(x,y,z) = M_pht_env*sigma_pht_env*env(x,y,z)
        end do
     end do
  end do
  ...
  do z = 1,psz
     do y = 1,psy
       ...
       !!! Calculate linear prefactor
           lnr(linindex) = (4*w_pht*env(x,y,z+1)*M_pht_env)

       !!! Calculate quadratic prefactor
           sqr(linindex) = (2*M_pht_env*w_pht) - (2*M_pht_env*hill_pht_env*env(x,y,z+1))
