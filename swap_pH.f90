subroutine swap_pH
  ...
  !! a) First number denotes field: 1 - pH
...
!!!!!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@!!!!!
  ...
  if ((rank.gt.0).and.(rank.lt.procs-1)) then
     call mpi_isend(pH(1,1,psz+1),psx*psy*ghost_width,MPI_DOUBLE_PRECISION,rank+1,10,MPI_COMM_WORLD,q111,ierr)
     call mpi_isend(pH(1,1,1+ghost_width),psx*psy*ghost_width,MPI_DOUBLE_PRECISION,rank-1,11,MPI_COMM_WORLD,q110,ierr)
     call mpi_irecv(pH(1,1,1),psx*psy*ghost_width,MPI_DOUBLE_PRECISION,rank-1,10,MPI_COMM_WORLD,q120,ierr)
     call mpi_irecv(pH(1,1,psz+1+ghost_width),psx*psy*ghost_width,MPI_DOUBLE_PRECISION,rank+1,11,MPI_COMM_WORLD,q121,ierr)
  ...
  elseif (rank.eq.0) then
     ...
     do x = 1,psx
        do y = 1,psy
           do z = 1,ghost_width
              pH(x,y,psz+ghost_width+z) = pH(x,y,psz+ghost_width)
           end do
        end do
     end do
