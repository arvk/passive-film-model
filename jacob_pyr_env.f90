subroutine J_Pyr_Env(snes,pyr_vec,pyr_jacob,pyr_precond,dummy,ierr)
  use commondata
  use fields
  use laplacians
  use gradients
...
#include <finclude/petscsnes.h>
  ...
  do x = 1,psx
     do y = 1,psy
        do z = 1,psz+2
           D_pyr(x,y,z) = M_pyr_env*sigma_pyr_env*env(x,y,z)
        end do
     end do
  end do
  ...
  do z = 1,psz
     do y = 1,psy
       ...
       !!! Calculate linear prefactor
           lnr(linindex) = (4*w_pyr*env(x,y,z+1)*M_pyr_env)

       !!! Calculate quadratic prefactor
           sqr(linindex) = (2*M_pyr_env*w_pyr) - (2*M_pyr_env*hill_pyr_env*env(x,y,z+1))
