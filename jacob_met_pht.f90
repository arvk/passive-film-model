subroutine J_Met_Pht(snes,met_vec,met_jacob,met_precond,dummy,ierr)
  use commondata
  use fields
  use laplacians
  use gradients
...
#include <finclude/petscsnes.h>
  ...
  do x = 1,psx
     do y = 1,psy
        do z = 1,psz+2
           D_met(x,y,z) = M_met_pht*sigma_met_pht*pht(x,y,z)
        end do
     end do
  end do
  ...
  do z = 1,psz
     do y = 1,psy
       ...
       !!! Calculate linear prefactor
           lnr(linindex) = (4*w_met*pht(x,y,z+1)*M_met_pht)

       !!! Calculate quadratic prefactor
           sqr(linindex) = (2*M_met_pht*w_met) - (2*M_met_pht*hill_met_pht*pht(x,y,z+1))
