subroutine J_Pht_Pyr(snes,pht_vec,pht_jacob,pht_precond,dummy,ierr)
  use commondata
  use fields
  use laplacians
  use gradients
...
#include <finclude/petscsnes.h>
  ...
  do x = 1,psx
     do y = 1,psy
        do z = 1,psz+2
           D_pht(x,y,z) = M_pht_pyr*sigma_pht_pyr*pyr(x,y,z)
        end do
     end do
  end do
  ...
  do z = 1,psz
     do y = 1,psy
       ...
       !!! Calculate linear prefactor
           lnr(linindex) = (4*w_pht*pyr(x,y,z+1)*M_pht_pyr)

       !!! Calculate quadratic prefactor
           sqr(linindex) = (2*M_pht_pyr*w_pht) - (2*M_pht_pyr*hill_pht_pyr*pyr(x,y,z+1))
