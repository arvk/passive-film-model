subroutine swap_or
  ...
  !! a) First number denotes field: 1 - opyr
  ...
  if ((rank.gt.0).and.(rank.lt.procs-1)) then
     call mpi_isend(opyr(1,1,psz+1),psx*psy*ghost_width,MPI_DOUBLE_PRECISION,rank+1,10,MPI_COMM_WORLD,q111,ierr)
     call mpi_isend(opyr(1,1,1+ghost_width),psx*psy*ghost_width,MPI_DOUBLE_PRECISION,rank-1,11,MPI_COMM_WORLD,q110,ierr)
     call mpi_irecv(opyr(1,1,1),psx*psy*ghost_width,MPI_DOUBLE_PRECISION,rank-1,10,MPI_COMM_WORLD,q120,ierr)
     call mpi_irecv(opyr(1,1,psz+1+ghost_width),psx*psy*ghost_width,MPI_DOUBLE_PRECISION,rank+1,11,MPI_COMM_WORLD,q121,ierr)
