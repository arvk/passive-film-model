subroutine J_Env_Pht(snes,env_vec,env_jacob,env_precond,dummy,ierr)
  use commondata
  use fields
  use laplacians
  use gradients
...
#include <finclude/petscsnes.h>
  ...
  do x = 1,psx
     do y = 1,psy
        do z = 1,psz+2
           D_env(x,y,z) = M_env_pht*sigma_env_pht*pht(x,y,z)
        end do
     end do
  end do
  ...
  do z = 1,psz
     do y = 1,psy
       ...
       !!! Calculate linear prefactor
           lnr(linindex) = (4*w_env*pht(x,y,z+1)*M_env_pht)

       !!! Calculate quadratic prefactor
           sqr(linindex) = (2*M_env_pht*w_env) - (2*M_env_pht*hill_env_pht*pht(x,y,z+1))
