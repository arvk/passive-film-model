...
all: spparks_filmenv.o spparks_metfilm.o
	$(FC) -o model.exe $(CFLAG) $(OFLAG) $(INC) shared_modules.f90 wrapper.f90 read_parameters.f90 distrib_params.f90 seed_prng.f90 initialize_geometry.f90 swap_pf.f90 pfsolve.f90 nonlin_feval.f90 nonlin_jacob.f90 musolve.f90 pHsolve.f90 orsolve.f90 elpotsolve.f90 electro_feval.f90 electro_jacob.f90 swap_electro.f90 dissolve_film.f90 voids_create.f90 write_parameters.f90 read_geometry.f90 gather_pf.f90 gather_mu.f90 gather_opyr.f90 gather_electro.f90 write_fields.f90 calc_grad_pf.f90 thermo.f90 spparks_filmenv.o spparks_metfilm.o swap_mu.f90 swap_pH.f90 swap_or.f90 allocate.f90 distrib_pf.f90 diffusivities.f90 estimate_timestep.f90 ${PETSC_LIB} ${SPPARKS_LIB}
