subroutine J_Pht_Met(snes,pht_vec,pht_jacob,pht_precond,dummy,ierr)
  use commondata
  use fields
  use laplacians
  use gradients
...
#include <finclude/petscsnes.h>
  ...
  do x = 1,psx
     do y = 1,psy
        do z = 1,psz+2
           D_pht(x,y,z) = M_pht_met*sigma_pht_met*met(x,y,z)
        end do
     end do
  end do
  ...
  do z = 1,psz
     do y = 1,psy
       ...
       !!! Calculate linear prefactor
           lnr(linindex) = (4*w_pht*met(x,y,z+1)*M_pht_met)

       !!! Calculate quadratic prefactor
           sqr(linindex) = (2*M_pht_met*w_pht) - (2*M_pht_met*hill_pht_met*met(x,y,z+1))
